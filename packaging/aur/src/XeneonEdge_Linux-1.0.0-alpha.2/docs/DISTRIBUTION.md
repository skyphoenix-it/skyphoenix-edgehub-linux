...
- After the first zsync-enabled release, embedding AppImageUpdate's
  `gh-releases-zsync` update info *inside* the AppImage is a possible
  follow-up; the plain `.zsync` asset works without it.
